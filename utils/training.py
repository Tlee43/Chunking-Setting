...
def evaluate(model: ContinualModel, dataset: ContinualDataset, last=False) -> Tuple[list, list]:
    ...
    for k, test_loader in enumerate(dataset.test_loaders):
        if last and k < len(dataset.test_loaders) - 1:
            continue
        correct, correct_mask_classes, total = per_task_eval(dataset, model, k, test_loader, class_means)
...
def per_task_eval(dataset, model, k, test_loader, class_means):
    ...
    return correct, correct_mask_classes, total
            ...
            for i, data in enumerate(train_loader):
                ...
                if args.per_it_eval:
                    if model.nmc:
                        ...
                        class_means = None
                    task_0_acc = per_task_eval(dataset, model, 0, dataset.test_loaders[0], class_means)
                    print(' task_0_acc='+str(task_0_acc[0]/task_0_acc[2]), end='', flush=True)
